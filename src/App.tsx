    ...
    <div className="min-h-screen bg-stone-50 text-stone-900">
      <header className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          {user?.role === 'admin' && (
            <button onClick={() => setIsAdminView(!isAdminView)} className={`p-2.5 rounded-xl transition-all ${isAdminView ? 'bg-emerald-600 text-white shadow-lg' : 'bg-stone-100'}`}><Settings size={22} /></button>
          )}
          {user && <button onClick={() => signOut(auth)} className="p-2.5 text-stone-400 hover:text-red-500"><LogOut size={22} /></button>}
        </div>
      </header>

      <main className="max-w-7xl mx-auto p-4 md:p-8">
        {!user ? (
          <div className="max-w-md mx-auto mt-20 bg-white p-12 text-center shadow-2xl rounded-[2rem] border border-stone-100 animate-in fade-in zoom-in duration-300">
          ...
          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
            <div className="flex gap-2 p-1.5 bg-stone-200/50 rounded-2xl w-fit">
              {(['menu', 'students', 'report'] as const).map(tab => (
                <button key={tab} onClick={() => setAdminTab(tab)} className={`px-8 py-2.5 rounded-xl font-bold transition-all ${adminTab === tab ? 'bg-white text-emerald-700 shadow-sm' : 'text-stone-500'}`}>
                  {tab === 'menu' ? '献立管理' : tab === 'students' ? '職員名簿' : '月間レポート'}
                </button>
              ))}
            </div>
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100 min-h-[500px]">
              {adminTab === 'menu' && (
                <div className="space-y-6">
                  <div className="flex justify-between items-center"><h3 className="text-xl font-black">献立一覧</h3><button className="flex items-center gap-2 bg-emerald-600 text-white px-6 py-3 rounded-2xl font-bold"><Plus size={20} /> 新規登録</button></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {menu.map(m => (
                      <div key={m.id} className="p-5 border border-stone-100 rounded-[1.5rem] hover:border-emerald-200 transition-all">
                        <div className="flex justify-between items-start mb-2"><span className="text-xs font-bold text-stone-400">{m.date}</span><span className={`px-2 py-0.5 rounded text-[10px] font-black ${m.meal_type === 'lunch' ? 'bg-orange-100 text-orange-600' : 'bg-indigo-100 text-indigo-600'}`}>{m.meal_type === 'lunch' ? '昼食' : '夕食'}</span></div>
                        <p className="font-bold text-lg mb-4">{m.title}</p>
                        <div className="flex gap-2"><button className="flex-1 py-2 bg-stone-50 rounded-xl text-xs font-bold">編集</button><button className="p-2 text-stone-300 hover:text-red-500"><Trash2 size={18} /></button></div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {adminTab === 'students' && (
                <div className="space-y-6">
                  <h3 className="text-xl font-black">登録職員一覧（{adminUsers.length}名）</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {adminUsers.map(u => (
                      <div key={u.id} className="p-5 bg-stone-50 rounded-2xl flex justify-between items-center font-bold">
                        <span>{u.name}</span><span className="text-[10px] bg-white px-3 py-1 rounded-full shadow-sm text-stone-400">{u.role === 'admin' ? '管理者' : '一般職員'}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {adminTab === 'report' && (
                <div className="space-y-8 text-center py-20">
                  <h3 className="text-2xl font-black">レポート出力</h3>
                  <div className="flex justify-center gap-4">
                    ...
                    <button className="bg-stone-800 text-white px-8 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-xl hover:bg-black transition-all active:scale-95"><Download size={20} /> CSVで書き出し</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-6">
              <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100">
                ...
                <div className="bg-emerald-50 rounded-[2rem] p-8 border border-emerald-100/50">
                  <div className="flex items-center justify-between mb-8">
                    <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-white px-6 py-3 rounded-2xl font-black text-emerald-800 shadow-sm border-none" />
                    <div className="flex bg-white/80 rounded-2xl p-1 shadow-sm">
                      ...
                      <button onClick={() => setSelectedMealType('dinner')} className={`px-6 py-2 rounded-xl text-sm font-black transition-all ${selectedMealType === 'dinner' ? 'bg-emerald-600 text-white shadow-lg' : 'text-emerald-600'}`}>夕食</button>
                    </div>
                  </div>
                  {menu.find(m => m.date === selectedDate && m.meal_type === selectedMealType) ? (
                    <div className="space-y-6">
                      <p className="text-3xl font-black text-stone-800 text-center">{menu.find(m => m.date === selectedDate && m.meal_type === selectedMealType)?.title}</p>
              ...
              <section className="bg-white p-6 rounded-[2rem] shadow-sm border border-stone-100">
                ...
                <p className="text-stone-500 font-bold">今日も美味しく食べて、午後のお仕事も頑張りましょう！</p>
              </section>
            </div>

            {/* 右：本日の喫食確認 */}
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100 h-fit">
              ...
              <div className="space-y-4">
                {reservations.filter(r => r.date === selectedDate).length > 0 ? (
                  reservations.filter(r => r.date === selectedDate).map(res => (
                    <div key={res.id} className="flex items-center justify-between p-6 bg-stone-50 rounded-[1.5rem] border border-stone-100 group transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <div><p className="font-black text-stone-800 text-lg leading-tight">{res.title}</p><p className="text-[10px] font-bold text-stone-300 uppercase mt-1">{res.meal_type === 'lunch' ? 'LUNCH' : 'DINNER'}</p></div>
                      </div>
                      <button onClick={() => toggleConsumed(res.id, res.consumed)} className={`px-8 py-3 rounded-2xl font-black text-sm transition-all shadow-sm ${res.consumed ? 'bg-emerald-600 text-white shadow-emerald-100' : 'bg-white text-emerald-600 hover:bg-emerald-50'}`}>
